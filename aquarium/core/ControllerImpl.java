...
public class ControllerImpl implements Controller {
    ...
    public String insertDecoration(String aquariumName, String decorationType) {
        Decoration currentDecoration = decorations.findByType(decorationType);
    ...
    public String addFish(String aquariumName, String fishType, String fishName, String fishSpecies, double price) {
        ...
        for (Aquarium aquarium : aquariums) {
            if (aquarium.getName().equals(aquariumName)) {
                String aqType = aquarium.getClass().getSimpleName();
                if ((aqType.startsWith("Fresh") && fishType.startsWith("Fresh")) ||
                        (aqType.startsWith("Salt") && fishType.startsWith("Salt"))) {
    ...
    public String calculateValue(String aquariumName) {
        double totalPrice = 0;
        Aquarium aquarium1 = aquariums.stream()
                .filter(aquarium -> aquarium.getName().equals(aquariumName))
                .findFirst().orElse(null);

        assert aquarium1 != null;
        Collection<Fish> fish = aquarium1.getFish();
        for (Fish f : fish) {
            totalPrice += f.getPrice();
        }

        Collection<Decoration> decorations = aquarium1.getDecorations();
